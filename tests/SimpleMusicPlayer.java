...
public class SimpleMusicPlayer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        server.createContext("/", new StaticFileHandler()); // Serve static files
        server.createContext("/songs", new SongsHandler()); // Serve songs in HTML
        server.createContext("/play", new SongStreamHandler()); // Stream songs
    ...
    // Insert songs from the directory into the database
        ...
        if (files != null) {
            for (File file : files) {
                try {
                    ...
                    // Use file name as the title if no metadata is found
                    if (title.isEmpty()) {
                        title = file.getName();
                    }
                    if (artist.isEmpty()) {
                    ...
                    // Insert the song into the database if it doesn't already exist
