            ...
            for (File song : songFiles) {
                if (song.isFile()) {
                    try {
                        ...
                        String releaseDate = "2023-01-01"; // Placeholder date, Jaudiotagger doesn't fetch dates

                        // Insert metadata into the Songs table
                        String insertSQL = "INSERT INTO Songs (title, artist, genre, release_date, duration, song_file_path) "
                                + "VALUES (?, ?, ?, ?, ?, ?)";
